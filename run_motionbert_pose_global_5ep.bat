...
rem =========================
rem MotionBERT 3D Pose Runner (Global 5-Epoch Test Checkpoint)
rem This runner is isolated from the official checkpoint and output path.
...
set "OUTPUT_DIR=D:\Git\Github\MotionBERT\outputs\global_5ep_test"

rem Test config/checkpoint produced by the 5-epoch rootrel=False training run.
set "CONFIG=configs\pose3d\MB_train_h36m_global_5ep.yaml"
set "CHECKPOINT=checkpoint\pose3d\MB_train_h36m_global_5ep\best_epoch.bin"
...
cd /d "%SCRIPT_DIR%"
echo [INFO] Running MotionBERT 3D pose inference with global 5-epoch test checkpoint...
...
"%PYTHON_EXE%" infer_wild.py --config "%CONFIG%" --vid_path "%VIDEO_PATH%" --json_path "%JSON_PATH%" --out_path "%OUTPUT_DIR%" -e "%CHECKPOINT%" !EXTRA_ARGS!
