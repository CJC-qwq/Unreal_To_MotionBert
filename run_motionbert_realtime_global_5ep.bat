...
rem Global 5-epoch test checkpoint
...
rem MotionBERT config/checkpoint for the 5-epoch rootrel=False test model.
if not defined CONFIG set "CONFIG=configs/pose3d/MB_train_h36m_global_5ep.yaml"
if not defined CHECKPOINT set "CHECKPOINT=checkpoint/pose3d/MB_train_h36m_global_5ep/best_epoch.bin"
...
cd /d "%SCRIPT_DIR%"
echo [INFO] Running MotionBERT realtime 3D with global 5-epoch test checkpoint...
...
"%PYTHON_EXE%" realtime_motionbert.py --source "%SOURCE%" --camera_width %CAMERA_WIDTH% --camera_height %CAMERA_HEIGHT% --camera_fps %CAMERA_FPS% --min_frames %MIN_FRAMES% --config "%CONFIG%" --evaluate "%CHECKPOINT%" !EXTRA_ARGS!
