...
class SharedLatest2D:
    ...
    def update(self, person_frame, display_left, frame_index):
        with self.lock:
            self.person_frame = None if person_frame is None else person_frame.copy()
            self.display_left = None if display_left is None else display_left.copy()
            self.frame_index = frame_index
    ...
    def get(self):
        with self.lock:
            person_frame = None if self.person_frame is None else self.person_frame.copy()
            display_left = None if self.display_left is None else self.display_left.copy()
            return person_frame, display_left, self.frame_index, self.ended
...
def main():
        ...
        try:
            while True:
                ...
                if person is not None:
                    person_frame = person_to_h36m(person)
                else:
                    person_frame = empty_pose_frame()
                ...
                shared_2d.update(person_frame, display_left, frame_index)
                last_processed_index = frame_index
        finally:
            shared_2d.mark_ended()
            stop_event.set()
    ...
    def pose3d_loop():
        ...
        try:
            while True:
                person_frame, display_left, frame_index, ended = shared_2d.get()
                ...
                sequence_buffer.append(person_frame)
                ...
                if need_visual_output and display_left is not None:
    ...
    pose2d_thread = threading.Thread(target=pose2d_loop, name="pose2d_loop", daemon=True)
    pose3d_thread = threading.Thread(target=pose3d_loop, name="pose3d_loop", daemon=True)
    ...
    try:
        while True:
            if stop_event.is_set() and not pose2d_thread.is_alive() and not pose3d_thread.is_alive():
                break
        ...
        pose2d_thread.join(timeout=2.0)
        pose3d_thread.join(timeout=2.0)
        if livelink_thread is not None:
