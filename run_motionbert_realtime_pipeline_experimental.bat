...
rem Experimental pipeline version
...
if not exist "%SCRIPT_DIR%versions\realtime\realtime_motionbert_pipeline_experimental.py" (
    echo [ERROR] Experimental realtime script not found in:
    echo         %SCRIPT_DIR%versions\realtime\realtime_motionbert_pipeline_experimental.py
    set "EXIT_CODE=1"
    goto :end
)
...
cd /d "%SCRIPT_DIR%"
echo [INFO] Running MotionBERT realtime 3D experimental pipeline version...
...
"%PYTHON_EXE%" versions\realtime\realtime_motionbert_pipeline_experimental.py --source "%SOURCE%" --camera_width %CAMERA_WIDTH% --camera_height %CAMERA_HEIGHT% --camera_fps %CAMERA_FPS% --min_frames %MIN_FRAMES% !EXTRA_ARGS!
